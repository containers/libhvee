...
type MemorySettings struct {
	...
	// MaximumPerNumaNode specifies the maximum memory per NUMA node
	// in the virtual machine.
	MaximumPerNumaNode uint64 `json:"MaximumPerNumaNode,omitempty"`
...
func getMemoryCLI(vmName string, settings *MemorySettings) []string {
	...
	if settings.MaximumPerNumaNode > 0 {
		params = append(params, "-MaximumPerNumaNode", fmt.Sprintf("%d", settings.MaximumPerNumaNode))
	}
...
func GetVMMemoryFromName(vmName string) (*MemorySettings, error) {
	...
	if val, ok := memoryData["MaximumPerNumaNode"].(float64); ok {
		settings.MaximumPerNumaNode = uint64(val)
	}
