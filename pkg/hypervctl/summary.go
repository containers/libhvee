...
type PowerShellVM struct {
	...
	VMIntegrationService                []PowerShellVMIntegrationService `json:"VMIntegrationService,omitempty"`
...
func GetVMFromPowerShell(name string) (*PowerShellVM, error) {
	stdout, stderr, err := powershell.Execute("Get-VM", "-Name", name, "|", "ConvertTo-Json", "-Compress")
	if err != nil {
		return nil, NewPSError(stderr)
	}
